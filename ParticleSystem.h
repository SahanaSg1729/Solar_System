...
/* Solar energetic particles(are high energy particle coming from the sun, consist of protons electrons and HZEions.*/
/*Particles gives burning effect for sun*/
...
Particle ParticleSystem::generateNewParticle() {
	...
	return newPart; // returns the initialized values for all the variables of the structure Particle
}

Particle ParticleSystem::getNextParticle() {
	currentParticle = (currentParticle == NUMBER_OF_PARTICLES) ? 0 : currentParticle;
	return particles[currentParticle++]; //returns next particle
}

int ParticleSystem::getNumberOfParticles() {
	return NUMBER_OF_PARTICLES;  //returns 3000
}
...
float ParticleSystem::generateRandomNumber(float lower, float upper) {
	...
	if (firstTime) {
		time(&randomNumberSeed); //caluculates the number of seconds from jan 1st 1970 till the current time 
		srand(randomNumberSeed); //It sets the starting point for producing a series of pseudo-random integers
		firstTime = false;
	}
	return (lower + ((upper - lower) * (float(rand()) / RAND_MAX)));
	//Rand() function will generate random numbers in the range[0,RAND_MAX]
	//Formul(a+((b-a)*(float(rand()) / RAND_MAX)));(Ex:a=0,b=360)
}
